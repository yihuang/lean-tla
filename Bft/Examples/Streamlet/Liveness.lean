...
theorem Blk.epoch.ne_nil_tail_of_pos {b : Blk} (hv : ValidChain b) (hpos : 0 < b.epoch) :
    b.tail ≠ [] := by
  have h0t : 0 ∈ b.tail := Blk.epoch.mem_tail_of_pos hv.1 hpos
...
theorem Blk.epoch.length_ge_two_of_pos {b : Blk} (hv : ValidChain b) (hpos : 0 < b.epoch) : 2 ≤ b.length := by
  have h0t : 0 ∈ b.tail := Blk.epoch.mem_tail_of_pos hv.1 hpos
...
`proposalParentNotarizedBy`) takes exactly these two facts, so the liveness theorems
...
that epoch (`voteOnLeaderProposal` + `propUniq`). -/
theorem voted_eq_proposal_of_epoch {s : St n} (hinv : Inv n Byz Δ f L s)
    {e₀ : ℕ} {b : Blk} {i : Fin n} {C : Blk}
    (hp : HonestProposal n Byz L s e₀ b)
    (hih : i ∉ Byz) (hv : (i, C) ∈ s.castVotes) (hbe : C.epoch = e₀) :
    C = b := by
  have hCP : (L (C.epoch), C.epoch, C) ∈ s.castProps := hinv.voteOnLeaderProposal i C hih hv
  have hCb : (L e₀, e₀, C) ∈ s.castProps := by simpa [hbe] using hCP
  exact hinv.propUniq e₀ C b hCb hp.mem hp.honest

/-- A non-genesis honest vote for `C` with `C.epoch < e` is no longer than the
honest `e+1` proposal: `C`'s tail is notarized by `C.epoch - 1 ≤ e`, so
...
theorem voted_le_proposal_of_earlier_epoch {s : St n} (hinv : Inv n Byz Δ f L s)
    ...
    (hCval : ValidChain C) (hCn0 : C ≠ [0]) (hlt : C.epoch < e) :
    C.length ≤ b1.length := by
  have hCn : C ≠ [] := ne_nil_of_valid hCval
  have hCpos : 0 < C.epoch := by
    by_contra hnot
    have h0 : C.epoch = 0 := Nat.eq_zero_of_not_pos hnot
    exact hCn0 (ValidChain.eq_genesis hCval h0)
  have hCn' : C.tail ≠ [] := Blk.epoch.ne_nil_tail_of_pos hCval hCpos
  have hCp : ChainNotarizedBy n Byz f s C.tail (C.epoch - 1) :=
    hinv.voteParentNotarizedBy i C hih hv
  have hN : NotarizedBy n Byz f s C.tail (C.epoch - 1) :=
    chain_notarized_block n Byz f hCp hCn'
  have hN' : NotarizedBy n Byz f s C.tail e :=
    notarizedBy_mono n Byz f (Nat.le_trans (Nat.sub_le (C.epoch) 1) (le_of_lt hlt)) hN
...
theorem proposal_le_voted_of_later_epoch {s : St n} (hinv : Inv n Byz Δ f L s)
  ...
  have hlong : ∀ C' : Blk, NotarizedBy n Byz f s C' (C.epoch - 1) → C'.length ≤ C.tail.length :=
  ...
  have hle' : e + 2 ≤ C.epoch - 1 := by omega
  have hb2N' : NotarizedBy n Byz f s b2 (C.epoch - 1) := notarizedBy_mono n Byz f hle' hb2N
  exact hlong b2 hb2N'
...
strictly earlier (`C.epoch < e`), or strictly later (`e + 2 < C.epoch`). This is
...
theorem vote_case_of_proposals {s : St n} (hinv : Inv n Byz Δ f L s)
    ...
    (hp2 : HonestProposal n Byz L s (e + 2) b2)
    (hih : i ∉ Byz) (hv : (i, C) ∈ s.castVotes) :
    C = b0 ∨ C = b1 ∨ C = b2 ∨ C.epoch < e ∨ e + 2 < C.epoch := by
  ...
  · by_cases h2 : C.epoch = e + 1
    ...
    · by_cases h3 : C.epoch = e + 2
      ...
      · have hsplit : C.epoch < e ∨ e + 2 < C.epoch := by omega
...
theorem longest_chain_by (hB : Byz.card ≤ f) {s : St n} (hinv : Inv n Byz Δ f L s)
  ...
  · subst C; omega
  · subst C; omega
  · -- `C.epoch < e`: an older vote; bound `C` by `b₁` via `propLongest`
    by_cases hg : C = [0]
    · subst C
      have hb2n : b2 ≠ [] := ne_nil_of_valid (hinv.propValid (e + 2) b2 hp2.mem hp2.honest)
  ...
  · omega  -- `e + 2 < C.epoch` contradicts `hbepC : C.epoch ≤ e + 2`
...
theorem main_liveness_lemma (hB : Byz.card ≤ f) {s : St n} (hinv : Inv n Byz Δ f L s)
  ...
  · -- earlier: `C ≤ b₁ < b₂ = C`, contradiction
    have hb2ge2 : 2 ≤ b2.length := by
      have hb2pos : 0 < b2.epoch := by
        have h : b2.epoch = e + 2 := hinv.proposedEpoch (e + 2) b2 hp2.mem hp2.honest
        omega
      exact Blk.epoch.length_ge_two_of_pos (hinv.propValid (e + 2) b2 hp2.mem hp2.honest) hb2pos
  ...
  · -- later: `e+2 < C.epoch`, so `b₂ ≤ C.tail < C`
...
def Finalized (s : St n) (e : ℕ) (b0 b b2 : Blk) : Prop :=
  0 < e ∧ b0.epoch = e ∧ Consecutive b0 b b2 ∧ ChainNotarizedCast n Byz f s b2
...
longest chain notarized by `e+2` (`proposalParentNotarizedBy`), and the parent is
...
theorem proposal_parent_notarized {s : St n} (hinv : Inv n Byz Δ f L s)
    ...
    (hb3pos : 0 < b3.epoch) : NotarizedBy n Byz f s b3.tail (e + 2) := by
  have hc : ChainNotarizedBy n Byz f s b3.tail (e + 2) :=
    hinv.proposalParentNotarizedBy (e + 3) b3 hp3.mem hp3.honest
  exact chain_notarized_block n Byz f hc
    (Blk.epoch.ne_nil_tail_of_pos (hinv.propValid (e + 3) b3 hp3.mem hp3.honest) hb3pos)
...
theorem next_proposal_extends (hB : Byz.card ≤ f) {s : St n} (hinv : Inv n Byz Δ f L s)
  ...
  have hb2pos : 0 < b2.epoch := by
    have h : b2.epoch = e + 2 := hinv.proposedEpoch (e + 2) b2 hp2.mem hp2.honest
    omega
  have hb3pos : 0 < b3.epoch := by
    have h : b3.epoch = e + 3 := hinv.proposedEpoch (e + 3) b3 hp3.mem hp3.honest
    omega
  have hb2ge2 : 2 ≤ b2.length :=
    Blk.epoch.length_ge_two_of_pos (hinv.propValid (e + 2) b2 hp2.mem hp2.honest) hb2pos
  ...
  have hb3eq : b3 = b3.epoch :: b3.tail := by
    cases b3 with
    | nil => exact (hb3n rfl).elim
    | cons a t => rfl
  have hb3E : b3.epoch = e + 3 := hinv.proposedEpoch (e + 3) b3 hp3.mem hp3.honest
...
theorem liveness_finality (hB : Byz.card ≤ f) {s : St n} (hinv : Inv n Byz Δ f L s)
  ...
  have hbep2 : b2.epoch = e + 2 := hinv.proposedEpoch (e + 2) b2 hp2.mem hp2.honest
  have hbep3 : b3.epoch = e + 3 := hinv.proposedEpoch (e + 3) b3 hp3.mem hp3.honest
  have hbep4 : b4.epoch = e + 4 := hinv.proposedEpoch (e + 4) b4 hp4.mem hp4.honest
