...
partial def liftFormula (stx : TSyntax `term) : MacroM (TSyntax `term) :=
  ...
  | `(∃ $x:ident, $b) => do
      ...
      `(Bft.tlaExists (fun $x:ident => $lb))
  | `(∃ $x:ident : $T, $b) => do
      let lb ← liftFormula b
      `(Bft.tlaExists (fun $x:ident : $T => $lb))
  | `(∃ $x:ident ∈ $S, $b) => do
      let lb ← liftFormula b
      `(Bft.tlaExists (fun $x:ident => $x ∈ $S ∧ $lb))
  | `(∀ $x:ident, $b) => do
      let lb ← liftFormula b
      `(Bft.tlaForall (fun $x:ident => $lb))
  | `(∀ $x:ident : $T, $b) => do
      let lb ← liftFormula b
      `(Bft.tlaForall (fun $x:ident : $T => $lb))
  | `(∀ $x:ident ∈ $S, $b) => do
      let lb ← liftFormula b
      `(Bft.tlaForall (fun $x:ident => $x ∈ $S → $lb))
  | `($a ∧ $b) => do
...
elab "[t| " body:term "]" : term <= expectedType => do
  ...
  let expected := mkApp (mkConst ``Bft.Pred [u]) σ
...
macro_rules
  | `([c| $Byz, $p | $body]) => `(Bft.CorrectAct $Byz $p [a| $body])
