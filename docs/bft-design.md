...
`leadsTo` has one. The class's consumer is `specSim_entails_stutter` (§5):
`StutterEq` (the equivalence generated by single stutter insertions and
legal deletions) is proved an equivalence in `Bft/Stutter.lean`, and
`SI.congr_stutterEq` transports satisfaction of any `SI` formula across it.
...
- `specSim_entails_stutter`: the formula-level closure — any behavior
  ...
  every stutter-invariant (`SI`) high-level consequence. First consumer of
  the `SI` typeclass: implementation = spec, up to stuttering.
