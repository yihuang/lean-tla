...
| 12 | Pattern library + tutorials (counter → mutex → 2PC → Paxos-safety → Paxos→Consensus refinement) | counter, mutex, ticket lock, 2PC, Paxos-safety done (`TlaDsl/Examples/Paxos.lean`: Voting-spec port, `ShowsSafety` theorem, agreement invariant); refinement done (`TlaDsl/Examples/PaxosConsensus.lean`: Voting→Consensus via `refinement_mapping_inv`); elaborator fix for `some v`/polymorphic apps in brackets | all |
